...
// Version identity is conveyed by a neutral letter badge (see VersionBadge below),
// never by color — color is reserved exclusively for meaning: sage = success,
// burgundy = failure/worst, amber = pending/incomplete. These selection styles
// are shared by both versions so neither reads as "more correct" than the other.
const NEUTRAL_SELECTION = {
  optionActive: 'bg-gold/10 border-gold/50 text-gold-deep font-bold',
  radio: 'accent-gold',
  focus: 'focus:border-gold',
} as const;

function VersionBadge({ version }: { version: 'A' | 'B' }) {
  return (
    <span className="w-5 h-5 rounded-full bg-paper-sunk border border-line text-ink-soft text-[11px] font-bold flex items-center justify-center shrink-0 font-latin">
      {version}
    </span>
  );
}
                  ...
                  <div className="p-3 bg-paper-sunk border border-line rounded-lg font-latin text-xs text-ink-soft max-h-[220px] overflow-y-auto space-y-2 text-left" dir="ltr">
                    ...
                    {section.tracks.A.clip_id && (
                      <div className="border-t border-line pt-2">
                        <span className="text-gold-deep font-bold block mb-1">Clip ID A:</span>
                        <p className="text-[10px] text-ink-faint font-bold">{section.tracks.A.clip_id}</p>
                      </div>
                    )}
                    {section.tracks.B.clip_id && (
                      <div className="border-t border-line pt-2">
                        <span className="text-gold-deep font-bold block mb-1">Clip ID B:</span>
      ...
      {project.sectionQuestions.length > 0 && (
        ...
        >
          {project.sectionQuestions.map((q) => {
            ...
            const isFullyAnswered = answer.A !== null && answer.B !== null;
            return (
              <div
                key={q.id}
                className={`bg-paper-raised rounded-2xl p-6 space-y-4 ${
                  isFullyAnswered ? 'border border-line' : 'border-2 border-dashed border-amber/40'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {!isFullyAnswered && (
                    <span className="text-[11px] text-amber bg-amber/10 border border-amber/40 px-2.5 py-1 rounded-full font-bold shrink-0 whitespace-nowrap">
                      بانتظار الإجابة
                    </span>
                  )}
                </div>
                  ...
                  {(['A', 'B'] as const).map((version) => {
                    ...
                    return (
                      <div key={version} className="p-4 rounded-xl bg-paper-sunk border border-line space-y-3">
                        <h4 className="text-xs font-bold text-ink-soft flex items-center gap-1.5">
                          <VersionBadge version={version} /> النسخة {version}:
                        </h4>
                          ...
                          {options.map((opt) => (
                            <label
                              ...
                              className={`flex items-center gap-2.5 p-2 rounded-lg text-xs cursor-pointer border transition-all ${
                                currentValue === opt
                                  ? NEUTRAL_SELECTION.optionActive
                                  : currentValue === null
                                  ? 'bg-paper-raised border-dashed border-amber/30 text-ink-soft hover:text-ink'
                            ...
                            >
                              <input
                                ...
                                className={NEUTRAL_SELECTION.radio}
                              />
                        ...
                        {showDetails && (
                          <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }}>
                            ...
                            <input
                              ...
                              className={`w-full bg-paper-raised border border-line rounded-lg px-3 py-1.5 text-xs text-ink focus:outline-none ${NEUTRAL_SELECTION.focus} font-body`}
                            />
                          </motion.div>
                        )}
                      </div>
        ...
        <div className="bg-paper-raised border border-line rounded-2xl overflow-hidden">
          ...
          <div>
            {section.tags.map((tag, idx) => {
              ...
              const isUntouched = tagRating.ratingA === null && tagRating.ratingB === null;
              return (
                <div
                  key={tag.id}
                  id={`tag-row-${tag.id}`}
                  className={`p-5 border-t border-line hover:bg-paper-sunk/40 transition-all flex flex-col lg:flex-row lg:items-center justify-between gap-4 ${
                    isUntouched ? 'border-r-4 border-r-amber/50' : ''
                  }`}
                >
                  <div className="space-y-1.5 lg:max-w-[45%]">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isUntouched && (
                        <span className="text-[10px] text-amber bg-amber/10 border border-amber/40 px-2 py-0.5 rounded-full font-bold">
                          غير مقيّم
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-6 shrink-0 bg-paper-sunk p-3 rounded-xl border border-line">
                    {(['A', 'B'] as const).map((version) => (
                      <div key={version} className="space-y-2">
                        <span className="font-latin italic text-xs text-ink-faint flex items-center gap-1">
                              ...
                              <button
                                ...
                                className={`w-[30px] h-[30px] rounded-full border flex items-center justify-center transition-all cursor-pointer ${
                                  isActive
                                    ...
                                    : isUnrated
                                    ? 'border-dashed border-amber/40 bg-paper-raised text-ink-faint'
