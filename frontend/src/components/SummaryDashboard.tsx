...
export default function SummaryDashboard({ project, evaluations, onImportState, onReset }: SummaryDashboardProps) {
  ...
  // how many sections landed on each answer, per version. Works for any
  // sectionQuestions schema.
  ...
  // option in ratingScale, by convention) show up here regardless of what
  // that rating is called in a given project.
...
const generateMarkdownReport = () => {
  ...
  return (
    <div id="summary-dashboard" className="space-y-8 text-right dir-rtl font-body pb-12">
      ...
      {/* Main Final Summary Table — columns generated from project.sectionQuestions */}
                  ...
                  <tr key={sec.id} id={`summary-row-${sec.id}`} className="hover:bg-paper-sunk/50 transition-all">
                    ...
                    {project.sectionQuestions.map((q) => {
                      ...
                      const renderVal = (val: string | null | undefined) => {
                        ...
                        const isWorst = val === (q.options || [])[1]; // convention: "bad" option is index 1 (e.g. 'yes' for drift, 'sudden' for transitions)
                        return (
      ...
      {/* Cross-section insights — computed generically from the project's own schema */}
              ...
              <div className="p-4 bg-paper-sunk border border-line rounded-xl space-y-2">
                {Object.entries(counts).map(([opt, c]) => (
                  <div key={opt} className="flex items-center justify-between text-xs text-ink-soft">
                    ...
                    <span className="font-latin text-ink-soft font-bold">A: {c.A} · B: {c.B}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-paper-raised border border-line rounded-2xl p-6 flex flex-col justify-between lg:col-span-2">
          <div className="space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <h4 className="font-display text-base text-ink font-bold">تاقات بحاجة لمراجعة دقيقة (حصلت على أسوأ تقييم متكرر)</h4>
            </div>
            {flaggedTags.length === 0 ? (
              <p className="text-xs text-ink-soft">لا توجد تاقات حصلت على أسوأ تقييم حتى الآن. جميع التاقات سليمة ومتزنة. ✅</p>
            ) : (
      ...
      <div className="bg-paper-raised border border-line rounded-2xl p-8 text-center space-y-6">
        <div className="max-w-xl mx-auto space-y-2">
          ...
          <p className="text-sm text-ink-soft leading-relaxed">
            انسخ التقرير كاملاً بصيغة ماركداون لمشاركته، أو صدّر/استورد حالة العمل الخام كملف JSON.
          </p>
        </div>
